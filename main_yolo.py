...
def main_yolo(dataset='_a1'):
    ...
    for model_name in models_to_test:
        ...
        for seed in seeds:
            ...
            eval_csv_path = f"{results_folder}/{model_name}_evaluation_metrics_5_seeds.csv"
            pd.DataFrame(all_eval_metrics).to_csv(eval_csv_path, index=False)

            # 3. Copy results
            results_csv = os.path.join(run_dir, "results.csv")
            destination_csv = f"{results_folder}/{model_name}_seed_{seed}_train_results.csv"
            ...
            plot_training_results(
                ...
                save_path=f"{results_folder}/{model_name}_seed_{seed}_training_plot.png"
            )
